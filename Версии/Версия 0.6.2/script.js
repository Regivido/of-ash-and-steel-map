...
function saveToolsStates() {
    ...
    console.log('Состояния инструментов сохранены:', statesToSave);
}
...
/**
 * Миграция старых ID маркеров к новой системе
 */
function migrateOldMarkerIds() {
    try {
        const saved = localStorage.getItem('markedMarkers');
        if (!saved) return;
        
        const oldMarkedMarkers = JSON.parse(saved);
        const migratedMarkedMarkers = {};
        
        // Проверяем, используются ли старые ID (числовые)
        const hasOldIds = Object.keys(oldMarkedMarkers).some(key => !isNaN(key));
        
        if (!hasOldIds) {
            // Уже используем новые ID
            return;
        }
        
        console.log('Миграция старых ID маркеров...');
        
        // Очищаем старое состояние
        localStorage.removeItem('markedMarkers');
        markedMarkers = {};
        
        console.log('Миграция завершена, старые ID удалены');
        
    } catch (error) {
        console.error('Ошибка миграции ID маркеров:', error);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (checkbox || label) {
            ...
            const activeMarker = allMarkers.find(marker => marker.customId === markerId);
            if (!activeMarker) {
                console.error(`Маркер с ID ${markerId} не найден`);
                return;
            }
